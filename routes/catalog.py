...
def games():
    ...
    rows = db.execute("""
        ...
        LEFT JOIN user_game_list ug
            ON ug.appid = g.appid AND ug.user_id = %s
        WHERE og.steamid = %s
...
def save_all(appid):
    ...
    cur = db.execute("""
        UPDATE player_hours SET hours = %s
        WHERE steamid = %s AND appid = %s
    ...
    cur = db.execute("""
        UPDATE user_game_list SET rating = %s, notes = %s
        WHERE user_id = %s AND appid = %s
    ...
    for row in games_raw:
        ...
        member_rows = db.execute("""
            SELECT 
                ...
                ug.rating
            FROM users u
            LEFT JOIN player_hours ph ON ph.steamid = u.steamid AND ph.appid = %s
            LEFT JOIN user_game_list ug ON ug.user_id = u.id AND ug.appid = %s
